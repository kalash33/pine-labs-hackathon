...
const TEST_UPI_ACCOUNTS = [
  // ── Success ─────────────────────────────────────────────────────────────────
  { label: '✅ Success', id: 'rahul.sharma@okhdfc', bank: 'HDFC Bank', hint: 'HDFC Bank · GPay handle · 99.8% success rail — payment goes through' },
  // ── Failure scenarios ────────────────────────────────────────────────────────
  { label: '❌ UPI Timeout', id: 'priya.mehta@oksbi', bank: 'State Bank of India', hint: 'SBI · Bank gateway timed out (30s) → AI suggests Card or Wallet' },
  { label: '❌ Account Blocked', id: 'amit.verma@okaxis', bank: 'Axis Bank', hint: 'Axis Bank · UPI ID blocked by bank → AI recommends Card or Wallet' },
  { label: '❌ Insufficient Funds', id: 'neha.gupta@okhdfcbank', bank: 'HDFC Bank', hint: 'HDFC Bank · Linked account has low balance → AI suggests EMI (₹1,500×3)' },
  { label: '❌ Invalid VPA', id: 'rohit.kumar99@ybl', bank: 'Yes Bank (PhonePe)', hint: 'Yes Bank · VPA not registered with any bank → AI prompts to use Card' },
  { label: '❌ Daily Limit Exceeded', id: 'sunita.patel@icici', bank: 'ICICI Bank', hint: 'ICICI Bank · UPI daily limit (₹1,00,000) hit → AI suggests EMI or Card' },
];

// Maps UPI test IDs to error codes for the Recovery Agent
const UPI_FAIL_MAP: Record<string, string> = {
  'priya.mehta@oksbi': 'UPI_TIMEOUT',
  'amit.verma@okaxis': 'UPI_BLOCKED',
  'neha.gupta@okhdfcbank': 'INSUFFICIENT_FUNDS',
  'rohit.kumar99@ybl': 'UPI_INVALID_VPA',
  'sunita.patel@icici': 'UPI_LIMIT_EXCEEDED',
};
...
export default function CheckoutPage() {
  ...
  const [showTestUpi, setShowTestUpi] = useState(false);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      const upiErrorCode = activeMethod === 'upi' ? UPI_FAIL_MAP[upiId.trim().toLowerCase()] : null;
      ...
      } else if (activeMethod === 'card' && binInfo && binInfo.bankHealth === 'down') {
        setNetworkHealth({ status: 'risk', message: `🔴 OUTAGE: ${binInfo.issuer} is completely unreachable (${binInfo.successProbability}% success). Payment will fail. AI recommends switching to UPI or a different card.` });
      } else if (activeMethod === 'card' && binInfo && binInfo.bankHealth === 'degraded') {
        setNetworkHealth({ status: 'risk', message: `⚠️ ${binInfo.successProbability}% Success Risk: ${binInfo.issuer} is experiencing intermittent issues. High failure probability. Consider switching payment method.` });
      } else if (activeMethod === 'upi' && upiErrorCode === 'UPI_TIMEOUT') {
        setNetworkHealth({ status: 'risk', message: '⚠️ UPI Gateway Timeout: SBI UPI server is unreachable. Payment will fail. AI recommends Card or Wallet.' });
      } else if (activeMethod === 'upi' && upiErrorCode === 'UPI_BLOCKED') {
        setNetworkHealth({ status: 'risk', message: '🔴 UPI ID Blocked: This VPA is blocked by the issuing bank. Payment will fail. Switch to Card or Wallet.' });
      } else if (activeMethod === 'upi' && upiErrorCode === 'UPI_INVALID_VPA') {
        setNetworkHealth({ status: 'risk', message: '🔴 Invalid VPA: This UPI ID is not registered with any bank. Payment will fail. Use Card instead.' });
      } else if (activeMethod === 'upi' && upiErrorCode === 'UPI_LIMIT_EXCEEDED') {
        setNetworkHealth({ status: 'risk', message: '⚠️ UPI Daily Limit Reached: ₹1,00,000 limit exhausted. Payment will fail. AI recommends EMI or Card.' });
      } else if (activeMethod === 'upi' && upiErrorCode === 'INSUFFICIENT_FUNDS') {
        setNetworkHealth({ status: 'risk', message: '⚠️ Insufficient Balance: Linked account has low funds. Payment will fail. AI recommends EMI (₹1,500×3).' });
  ...
  }, [activeMethod, cardNumber, upiId, binInfo]);
  ...
  const handlePayment = async () => {
    ...
    if (activeMethod === 'card') {
      ...
      // Bank health failure — trigger Recovery Agent same as card decline
      if (binInfo && binInfo.bankHealth !== 'online') {
        setIsProcessing(true);
        setRecoveryErrorCode(binInfo.bankHealth === 'down' ? 'BANK_NETWORK_DOWN' : '504_BANK_TIMEOUT');
        setTimeout(() => { setIsProcessing(false); setShowRecovery(true); }, 2000);
        return;
      }
    } else if (activeMethod === 'upi') {
      const upiErrorCode = UPI_FAIL_MAP[upiId.trim().toLowerCase()];
      if (upiErrorCode) {
        setIsProcessing(true);
        setRecoveryErrorCode(upiErrorCode);
        setTimeout(() => { setIsProcessing(false); setShowRecovery(true); }, 2000);
        return;
      }
    }
            ...
            <Card className="border-0 bg-[#0c0c18] ring-1 ring-white/8 rounded-2xl overflow-hidden shadow-2xl shadow-black/50">
              ...
              <CardContent className="pt-5 space-y-5">
                ...
                {!paymentLoading && activeMethod === 'upi' && (
                  <div className="space-y-3">
                    {/* Header row with UAT button */}
                    <div className="flex items-center justify-between">
                      <p className="text-xs text-slate-500">Enter UPI ID</p>
                      <button
                        onClick={() => setShowTestUpi(!showTestUpi)}
                        className="flex items-center gap-1 text-xs text-indigo-400 hover:text-indigo-300 font-semibold bg-indigo-500/10 px-2.5 py-1 rounded-lg border border-indigo-500/20 transition-colors"
                      >
                        <Zap className="w-3 h-3" /> UAT Test UPI
                      </button>
                    </div>
                    ...
                    {showTestUpi && (
                      <div className="rounded-xl overflow-hidden border border-white/8 bg-[#0a0a14]">
                        <div className="px-3 py-2 border-b border-white/5 text-[10px] font-bold text-indigo-400 uppercase tracking-wider flex items-center gap-1.5">
                          <Zap className="w-3 h-3" /> Quick Fill — UAT Test UPI Accounts
                        </div>
                        {TEST_UPI_ACCOUNTS.map((acc, i) => (
                          <button key={i}
                            onClick={() => { setUpiId(acc.id); setShowTestUpi(false); }}
                            className="w-full text-left px-3 py-2.5 hover:bg-white/5 transition-colors border-b border-white/5 last:border-0 group">
                            <div className="flex items-center justify-between gap-2">
                              <div className="flex items-center gap-2 min-w-0">
                                <span className="text-sm font-semibold text-slate-200 group-hover:text-indigo-300 transition-colors shrink-0">{acc.label}</span>
                                <span className="text-[10px] text-slate-500 truncate">{acc.bank}</span>
                              </div>
                              <span className="text-[10px] font-mono text-slate-500 bg-black/40 px-2 py-0.5 rounded border border-white/5 shrink-0">{acc.id}</span>
                            </div>
                            <p className="text-[11px] text-slate-600 mt-1">{acc.hint}</p>
                          </button>
                        ))}
                      </div>
                    )}
                    ...
                    {/* Failure warning for known fail UPI IDs */}
                    {UPI_FAIL_MAP[upiId.trim().toLowerCase()] && (
                      <div className="flex items-start gap-2 p-3 bg-amber-500/8 border border-amber-500/20 rounded-xl text-xs text-amber-400">
                        <AlertTriangle className="w-4 h-4 shrink-0 mt-0.5" />
                        <span>This UAT UPI ID will simulate a payment failure and trigger the AI Recovery Agent.</span>
                      </div>
                    )}

                    {/* Success confirmation for the success test account */}
                    {upiId.trim().toLowerCase() === 'rahul.sharma@okhdfc' && (
                      <div className="flex items-center gap-2 p-3 bg-emerald-500/8 border border-emerald-500/20 rounded-xl text-xs text-emerald-400">
                        <CheckCircle2 className="w-4 h-4 shrink-0" />
                        <span>UAT success account — payment will go through on the 99.8% success rail.</span>
                      </div>
                    )}
                ...
                {!activeMethod ? (
                  ...
                  <Button
                    className={`w-full h-12 text-sm font-bold group transition-all duration-200 rounded-xl border-0 shadow-lg ${
                      ...
                      (activeMethod === 'card' && binInfo && binInfo.bankHealth !== 'online') ||
                      (activeMethod === 'upi' && !!UPI_FAIL_MAP[upiId.trim().toLowerCase()])
                  ...
                  >
                    {isProcessing ? 'Simulating failure...' :
                     ...
                     (activeMethod === 'card' && binInfo?.bankHealth === 'down') ? 'Pay Anyway (Bank Outage)' :
                     (activeMethod === 'card' && binInfo?.bankHealth === 'degraded') ? 'Pay Anyway (High Risk)' :
                     (activeMethod === 'upi' && !!UPI_FAIL_MAP[upiId.trim().toLowerCase()]) ? 'Pay Anyway (Will Fail)' :
